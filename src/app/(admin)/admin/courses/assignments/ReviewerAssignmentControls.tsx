...
import {
  assignCourseReviewerAction,
  disableCourseReviewerAssignmentAction,
  type ReviewerAssignmentActionState,
} from "./actions";

const ALLOWED_SCOPE_TYPES = [
  {
    value: PermissionScopeType.COURSE_VERSION,
    label: "Specific Course Version",
  },
  { value: PermissionScopeType.COURSE, label: "Entire Course" },
  { value: PermissionScopeType.ORGANIZATION, label: "Entire Organization" },
];

export function GrantReviewerPanel({
  prefillType,
  prefillValue,
}: {
  prefillType?: string;
  ...
  const initialState: ReviewerAssignmentActionState = { ok: false, message: "" };
  ...
  const [scopeType, setScopeType] = useState<string>(
    prefillType || PermissionScopeType.COURSE_VERSION,
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3>Assign Course Reviewer</h3>
          <p style={{ fontSize: "0.875rem", opacity: 0.8 }}>
            Allocates formal review authority for submitted courses within established 
          ...
          >
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="e.g. course-version-id"
                required
                disabled={isPending}
              />
            </div>
...
export function ReviewerStatusControl({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await disableCourseReviewerAssignmentAction(assignmentId, reason.trim());
      if (res.ok) {
        setIsEditing(false);
      } else {
